...
const server=http.createServer(async(req,res)=>{
 ...
 if(p==='/rest/v1/rpc/homework_action'||p==='/functions/v1/homework'){
 ...
 if(action==='review'){assert.equal(body.p_data.upload_id,'upload');submission.status=body.p_data.status;submission.feedback=body.p_data.feedback;data={ok:true};}
...
(async()=>{
 ...
 try{
 c=await connectCdp({cdpUrl:'http://127.0.0.1:9232',targetUrl:'https://it-class-tcwu.vercel.app/admin-drive.html'});page=await c.browser.newPage();await page.setCacheEnabled(false);
 ...
 await page.evaluate(()=>{const dt=new DataTransfer();dt.items.add(new File([new Uint8Array(9*1024*1024)],'canva.mp4',{type:'video/mp4'}));document.querySelector('input[type=file]').files=dt.files;});
 await page.click('form button');await page.waitForFunction(()=>document.getElementById('homework-status').textContent.includes('稍後'));
 assert.equal(offset,4*1024*1024);assert.equal(submission,null);
 ...
 assert.equal(offset,9*1024*1024);assert.equal(submission.status,'submitted');assert.equal(calls.filter(x=>x==='chunk').length,3);
 ...
 assert.deepEqual(tree,{passed:[1,1],reupload:[1,0],legacy:[2,1]});
 ...
 await page.evaluate(()=>Array.from(document.querySelectorAll('button')).find(n=>n.textContent==='查看收件與評比').click());await page.waitForSelector('#roster-list textarea');
 await page.type('#roster-list textarea','影片內容清楚');await page.evaluate(()=>Array.from(document.querySelectorAll('button')).find(n=>n.textContent==='評為過關').click());
 ...
 await safeScreenshot(page,{path:path.join(root,'tmp/homework-teacher-mobile.png'),fullPage:true});
 assert.deepEqual(errors,[]);console.log('PASS anonymous/student isolation, 9 MB video interrupted at 4 MB then resumed, persisted status, teacher review, tree deduplication, typing regression, no refocus fetch, mobile layout. Mock Auth/API only.');
