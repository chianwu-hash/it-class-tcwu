import { SUPABASE_ANON_KEY, SUPABASE_URL, getStoredAccessToken } from "./auth.js";
...
async function callControlRpc(functionName, payload, { accessToken = null } = {}) {
    const controller = new AbortController();
    const timeoutId = window.setTimeout(() => controller.abort(), CONTROL_TIMEOUT_MS);
    try {
        const headers = {
        ...
        if (!response.ok) {
            ...
            const error = new Error(detail || `RPC ${functionName} failed with ${response.status}`);
...
export function initClassroomLinkControl({
    ...
    async function load({ userTriggered = false } = {}) {
        ...
        try {
            const data = await callControlRpc("get_classroom_control", {
                p_grade: grade,
    ...
    async function toggle() {
        ...
        try {
            ...
            const data = await callControlRpc("admin_set_classroom_control", {
                p_grade: grade,
                p_week_code: weekCode,
                p_control_key: controlKey,
